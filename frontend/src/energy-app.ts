...
function validatePrice(value: string): { valid: boolean; error?: string } {
  ...
  if (isNaN(num) || num <= 0) {
    return { valid: false, error: 'Please enter a valid positive price' }
  }
  if (num > 1) {
    return { valid: false, error: 'Price too high (max: 1 ETH per kWh)' }
  }
  return { valid: true }
}
...
async function createOffer() {
  ...
  try {
    ...
    // Update balances and refresh offers list
    await updateBalances()
